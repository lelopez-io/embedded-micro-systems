/* ======================================================================= */
/* DSP_blk_eswap64_d.c -- Endian-swap a block of 64-bit values             */
...
unsigned  x_cn[NX2 + 2*PAD];
unsigned  r_cn[NX2 + 2*PAD];

unsigned  x_i[NX2 + 2*PAD];
unsigned  r_i[NX2 + 2*PAD];
...
{   
    ...
    for(nx = 2, i = 1; nx <= NX; i++, nx += 2) {
        ...
        /* -------------------------------------------------------------------- */
        printf("DSP_blk_eswap64\tIter#: %d\t", i);
...
clock_t time_cn(int nx)
{
    ...
    t_start = clock();
    DSP_blk_eswap64_cn(ptr_x_cn, ptr_r_cn, nx);
    DSP_blk_eswap64_cn(ptr_x_cn, NULL, nx);
    t_stop = clock();
...
clock_t time_i(int nx)
{
    ...
    t_start = clock();
    DSP_blk_eswap64(ptr_x_i, ptr_r_i, nx);
    DSP_blk_eswap64(ptr_x_i, NULL, nx);
    t_stop = clock();
    return t_stop - t_start;
}
