/* ======================================================================= */
/* DSP_recip16_d.h -- Reciprocal Fucntion                                  */
...
/* ======================================================================= */
/* Inform the compiler the arrays are word alligned                        */
/* ======================================================================= */
#pragma DATA_ALIGN(x, 4);
#pragma DATA_ALIGN(rfrac_c, 4);
#pragma DATA_ALIGN(rexp_c, 4);
#pragma DATA_ALIGN(rfrac_i, 4);
#pragma DATA_ALIGN(rexp_i, 4);
...
short rfrac_c[N + 2*PAD];
short rexp_c[N + 2*PAD];
short rfrac_i[N + 2*PAD];
short rexp_i[N + 2*PAD];

/* ======================================================================= */
/* Generate pointers to skip beyond array padding                          */
/* ======================================================================= */
const short *const ptr_x = x + PAD;
short *const ptr_rfrac_c = rfrac_c + PAD;
short *const ptr_rexp_c = rexp_c + PAD;
short *const ptr_rfrac_i = rfrac_i + PAD;
short *const ptr_rexp_i = rexp_i + PAD;
...
int main()
{
    ...
    for(n = 2, i = 1; n <= N; i++, n += 2) {
        ...
        /* =================================================================== */
        memset(rfrac_c, 0xA5A5, sizeof(rfrac_c));
        memset(rexp_c, 0xA5A5, sizeof(rexp_c));
        memset(rfrac_i, 0xA5A5, sizeof(rfrac_i));
        memset(rexp_i, 0xA5A5, sizeof(rexp_i));
        ...
        /* =================================================================== */
        printf("DSP_recip16\tIter#: %d\t", i);

        /* =================================================================== */
        /* Check the results arrays, and report any failures                   */
        /* =================================================================== */
        if (memcmp(rfrac_i, rfrac_c, sizeof(rfrac_c))) {
            fail++;
            printf("Result Failure (rfrac_i)\t");
        }
        else
            printf("Result Successful (rfrac_i)\t");
        if (memcmp(rexp_i, rexp_c, sizeof(rexp_c))) {
            fail++;
            printf("Result Failure (rexp_i)");
        }
        else
            printf("Result Successful (rexp_i)");
...
clock_t time_c(int n)
{
    ...
    t_start = clock();
    DSP_recip16_cn(ptr_x, ptr_rfrac_c, ptr_rexp_c, n);
    t_stop = clock();
...
clock_t time_i(int n)
{
    ...
    t_start = clock();
    DSP_recip16(ptr_x, ptr_rfrac_i, ptr_rexp_i, n);
    t_stop = clock();
    return t_stop - t_start;
}
