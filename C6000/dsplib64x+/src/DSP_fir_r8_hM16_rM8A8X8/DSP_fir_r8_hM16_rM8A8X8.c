...
/*                           All Rights Reserved.                          */
/* ======================================================================= */
#pragma CODE_SECTION(DSP_fir_r8_hM16_rM8A8X8, ".text:intrinsic");
...
{
    int i, j;
    int sum_3, sum_2, sum_1, sum_0, r_10, r_32;
    int x_BA, x_98, x_76, x_54, x_32, x_10, x_3A, x_18;
    int h_76, h_54, h_32, h_10, h_21, h_43, h_65, h_07;

    double h_3210, h_7654;
    double x_3210, x_7654, x_BA98;
    ...
    for (j = 0; j < nr; j += 4) {
        ...
        for (i = 0; i < nh; i += 8) {
            ...
            /*----------------------------------------------------------------*/
            h_7654 = _amemd8_const(&h[i + 4]);
            h_3210 = _amemd8_const(&h[i]);

            /*----------------------------------------------------------------*/
            /* Use aligned double word wide loads to access the data array    */
            /*----------------------------------------------------------------*/
            x_BA98 = _amemd8_const(&x[i + j + 8]);
            x_7654 = _amemd8_const(&x[i + j + 4]);
            x_3210 = _amemd8_const(&x[i + j]);
